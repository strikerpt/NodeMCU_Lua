...
function web_html()  
    ...
    buf = buf .. "<a href='?pin=LT'><button id='blt'>L</button></a><a href='?pin=M'><button id='bm'>M</button></a><a href='?pin=V'><button id='bv'>V</button></a>\n" 
    buf = buf .. "<h1> Robot : </h1>\n" 
    ...
    buf = buf .. "<style>\n"
    buf = buf .. "#bf, #bb, #bl, #br, #bs, #bv, #bm, #blt, #ba, #bml, #bt1, #bt2, #bt3, #bt4, #bwifi{font-size:10px; position:relative; height:50px; width:50px;}\n"
