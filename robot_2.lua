--Programme pour faire tourner le robot à droite ou à gauche tant qu'il soit à moins de 20cm de l'obstacle
--hv20180713.1551
...
function zmesure()
    if gpio.read(zecho)==1 then 
        ...
        if zlength<20 then 
            ...
            tmr.alarm(2, 500, tmr.ALARM_SINGLE, avance_robot) -- Pour qu'il arrête de tourner 
        end
    end
end

gpio.trig(zecho, "both", zmesure)
tmr.alarm(1, 200, tmr.ALARM_AUTO, zmesure_pulse) -- à chaque 200 ms il mesure la distance

--Robot avance
function avance_robot()
    --Pour qu'il tourne à gauche ou à droite de façon random
    t=math.random(1,2)
    print(t)
    motor_a(FWD, 60)
    motor_b(FWD, 60)
